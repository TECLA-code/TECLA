    ...
    for btn_idx in range(n):
        ...
        if cur and not prv:
            if btn_idx == kbd._fn_scale_btn:
                ...
                # Toggle immediat (com el latch)
                kbd.voice_lead_active = not getattr(kbd, 'voice_lead_active', False)
                if kbd.voice_lead_active:
                    kbd._vl_prev_chord = None  # començar net: el 1r acord fixa la referència
                    print("🎼 Conducció de veus ACTIVADA")
                else:
                    print("🎼 Conducció de veus DESACTIVADA")
