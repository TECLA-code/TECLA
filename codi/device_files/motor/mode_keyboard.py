...
# El camí calent no executa sentències `import`. `update()` corre 177 vegades
# per segon, i un `import` d'un mòdul que ja és a sys.modules costa 0,10 ms
# igualment: construeix el nom, el busca i en resol els atributs. No comprava
# res —`setup()` ja carrega aquests mòduls de manera immediata, amb la memòria
# lliure— i des que el motor viu congelat a la flash tampoc estalvia RAM.
#
# Només es hissen els del camí calent. La resta de delegacions d'aquest fitxer
# es queden mandroses a posta: són fredes i el que estalvien és temps
# d'arrencada.
from motor.kbd_pots import update_parameters as _fn_update_parameters
from motor.kbd_buttons import process_keyboard_buttons as _fn_process_buttons
from motor.kbd_looper import tick as _fn_looper_tick
from motor.kbd_arp import process_arpeggiator as _fn_process_arp
...
class KeyboardMode:
    ...
    def __init__(self, midi_out, config=None, config_manager=None):
        ...
        # Les altures que sona L'ARPEGIADOR. Van a part de `button_notes`
        # perquè no vénen de cap tecla: `_note_on(nota, -1)`. Sense aquest
        # conjunt, l'única manera que tenia l'arp d'apagar-se era
        # `stop_all_notes()`, que apaga TOT — el loop i el que toquis a sobre
        # inclosos. Vegeu stop_arp_notes().
        self.arp_sounding = set()
    ...
    def setup(self):
        ...
        # La recàrrega pot haver canviat QUÈ fa cada pot sense que ningú l'hagi
        # mogut. La memòria de potes ja mira la funció, però `setup()` també
        # reescriu estat derivat (escales, tonalitats, mapatges), i és més
        # barat oblidar-ho tot aquí que confiar que cada camí ho detecti.
        try:
            from motor.kbd_pots import oblida
            oblida(self)
        except Exception:
            pass
    ...
    def cleanup(self):
        ...
        if self._accomp is not None:
            try:
                from motor.accompaniment import stop as _accomp_stop
                _accomp_stop(self)
            except Exception:
                pass
    ...
    def pause_looper(self):
        ...
        if self.loop_state:
            try:
                from motor.kbd_looper import pause_for_panic
                pause_for_panic(self)
            except Exception:
                pass

    def stop_arp_notes(self):
        """Apaga NOMÉS el que sona per l'arpegiador.

        L'arp fa un pas cada `arp_speed` i abans cridava `stop_all_notes()`,
        que envia NoteOff a TOTES les notes actives. Amb un loop sonant o amb
        el músic tocant a sobre, cada pas de l'arp els tallava les notes —
        reportat al dispositiu: «quan loopejo l'arpegiador i vull tocar a
        sobre hi ha conflicte».

        Mateixa regla que ja governa el looper i el teclat (vegeu
        `_note_off_for_button`): MIDI no compta propietaris, o sigui que qui
        no és l'amo d'una altura no l'apaga. Aquí l'arp se salta:

          · les altures que una tecla sosté ara mateix (`button_notes`), i
          · les que sosté el loop, que ja té el seu note-off programat.

        Sempre en queda un que l'apaga, o sigui que cap nota no es penja.
        """
        if not self.arp_sounding:
            return
        ...
        _viva = set()
        for notes_set in self.button_notes.values():
            if notes_set:
                _viva |= notes_set
        ...
        self.arp_sounding.clear()

    def stop_all_notes(self):
        """Para totes les notes actives i neteja tot el tracking.

        És el PÀNIC: canvi de capa, STOP general, sortir del mode. Apaga tot
        el que sona sense mirar de qui és, i per això l'arpegiador ja no hi
        passa a cada pas (vegeu stop_arp_notes)."""
        # Aquí sota es posa `sustain_level` a 0 comptant que el pot de sustain
        # ho reimposi a la volta següent. Des que el pot quiet NO repeteix la
        # feina (motor/kbd_pots), aquell "ho tornarà a posar" ja no passa sol:
        # cal dir-li explícitament que oblidi el que recorda.
        try:
            from motor.kbd_pots import oblida
            oblida(self)
        except Exception:
    ...
    def update(self, pot_values, button_states):
        ...
        # Actualitzar paràmetres des dels potenciòmetres. Mentre una capa de
        # comandaments del MODE DE FONS és activa, els potes són seus i el
        # teclat els té congelats (kbd_fons).
        self._ultims_pots = pot_values
        if not getattr(self, '_pots_al_fons', None):
        ...
        # SENSE llesca: process_keyboard_buttons ja limita a 15 pel seu compte,
        # i `[:15]` era una llista nova a cada volta.
        self._process_keyboard_buttons(button_states)

        # Motor del looper (només quan hi ha un loop reproduint-se)
        if self.loop_state == 3:  # PLAYING
            _fn_looper_tick(self, time.monotonic())
        ...
        if _f is not None and _f.nom is not None:
            _f.tick(pot_values)
            
    def _update_parameters(self, pot_values, force_update=False):
        _fn_update_parameters(self, pot_values, force_update)
    
    def _apply_pot_function(self, pot_name, pot_value, force_update=False):
        from motor.kbd_pots import apply_pot_function
    ...
    def _apply_arp_pot_function(self, pot_name, pot_value, force_update=False):
        from motor.kbd_pots import apply_arp_pot_function
    ...
    def _generate_note_from_custom_scale(self, slot, btn_idx, custom_scale):
        from motor.kbd_notes import generate_note_from_custom_scale
    ...
    def _note_on(self, note, button_index):
        ...
        try:
            from motor.base_mode import _note_on_msg
            msg = _note_on_msg()
            msg.note = note
            ...
            if button_index >= 0:
                ...
                self.arp_sounding.add(note)
            if button_index < 0 and self.loop_state:
                # Nota de l'arpegiador (button_index == -1): capturar-la al
                # looper si està armat/gravant — permet loopejar l'arp
                from motor.kbd_looper import record_live_note
