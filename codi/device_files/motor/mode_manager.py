...
def _get_mode_info_from_registry(mode_name):
    from motor.mm_lifecycle import _get_mode_info_from_registry as _f
    return _f(mode_name)
...
class ModeManager:
    ...
    def _stop_current_mode(self):
        from motor.mm_cleanup import mm_stop_current_mode
        mm_stop_current_mode(self)

    def enter_modes_layer(self):
        from motor.mm_cleanup import mm_enter_modes_layer
        return mm_enter_modes_layer(self)

    def unload_all_modes(self):
        from motor.mm_cleanup import mm_unload_all_modes
        return mm_unload_all_modes(self)

    def stop_all_sound(self):
        from motor.mm_cleanup import mm_stop_all_sound
        mm_stop_all_sound(self)

    def emergency_stop_and_cleanup(self):
        from motor.mm_cleanup import mm_emergency_stop
        return mm_emergency_stop(self)

    def cleanup(self):
        from motor.mm_cleanup import mm_cleanup
