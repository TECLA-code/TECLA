...
def _arp_notes_for_button(kbd, btn_idx, scale_intervals, key_offset, scale_id):
    ...
    if fn_idx >= 0:
        try:
            from motor.kbd_notes import _apply_harmonic_fn
...
def process_arpeggiator(kbd, button_states, current_time):
    ...
    if not pressed_buttons:
        # (El Gate NO es processa aquí: es fa a process_keyboard_buttons, fora
        #  d'aquest `return`. Mentre hi era, deixar anar les tecles amb
        #  l'arpegiador actiu el congelava, i si el cicle havia quedat a la
        #  fase baixa el CC11 es quedava a zero: Expression zero, canal MUT.
        #  Les notes seguien sortint i la Pantalla les mostrava, o sigui que no
        #  ho deia res. Vegeu tests/test_gate_no_es_congela.py.)
        # NOMÉS a la transició (última tecla deixada anar). Abans això corria a
        # CADA volta mentre l'arpegiador estava actiu sense tecles premudes:
        # 500 CC64=0 per segon més dues llistes noves, i les notes que tocaves
        # sortien darrere d'aquella cua. Si ja està tot net, no hi ha res a fer.
        if (kbd.arp_notes or kbd.active_notes or kbd.arp_button_order
                or kbd.arp_index or kbd._sustain_pending):
            kbd.stop_arp_notes()
            kbd.arp_index = 0
            kbd.arp_notes = []
    ...
    if current_time - kbd.last_arp_time >= kbd.arp_speed:
        # NOMÉS les seves: `stop_all_notes()` apagava també el loop i el que
        # el músic toqués a sobre, a cada pas de l'arpegiador.
        kbd.stop_arp_notes()
