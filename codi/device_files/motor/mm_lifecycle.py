...
def _modes_dir():
    """Carpeta dels modes i del seu registre, acabada en barra.
    ...
    perquè el registre no es trobés i TOTES les tecles quedessin mortes.

    Abans es preferia la ruta d'aquest mòdul. Ara ja no serveix: aquest mòdul
    viu a `motor/`, i amb la UF2 de TECLA a `.frozen/motor/`, que no és ni tan
    sols una carpeta del sistema de fitxers. Apuntaria a un lloc on no hi ha
    cap mode. Per això ara es prova cada candidat OBRINT-LO: mana el primer
    que existeix de debò, i la ruta del mòdul queda com a segona opció —el
    germà `modes/` de la carpeta del motor— per al cas que la va motivar.
    """
    global _MODES_DIR
    if _MODES_DIR is None:
        _MODES_DIR = 'modes/'
        for cand in _candidats_modes():
            try:
                os.listdir(cand[:-1])
            except OSError:
                continue
...
def _candidats_modes():
    """Per ordre de preferència. CircuitPython no porta os.path: va embolcallat."""
    yield 'modes/'
    try:
...
def mm_config_efectes(mgr, efectos):
    ...
    from motor.mm_update import mm_deactivate_efecte_temporal, _clear_susp_flags
...
def mm_set_mode(mgr, mode_name, force_reload=False, capture_state=True):
    ...
    try:
        ...
        # Comandaments: la llista que l'app ha desat per a aquest mode (si
        # n'hi ha) i les capes de 'Config Modes' reresoltes contra el mode
        # nou. La capa activa sobreviu al canvi (és el cas dels loops: canviar
        # de mode i seguir tenint el volum a mà) i es diu què hi ha ara.
        try:
            if hasattr(mgr.current_mode, 'set_comandaments') and mgr.config_manager:
                mgr.current_mode.set_comandaments(mgr.config_manager.get_comandaments(mode_name))
            _lay = getattr(mgr, '_potcfg', None)
            if _lay is not None:
