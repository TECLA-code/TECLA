...
def _route_audio_pots(mgr, preset, pot_values):
    ...
    cache = getattr(mgr, '_audio_cc_cache', None)
    if cache is None:
        cache = mgr._audio_cc_cache = {}
...
def _modeloop(mgr):
    ...
    lp = getattr(mgr, '_modeloop', None)
    if lp is None:
        from motor.modeloop import ModeLoop
        lp = ModeLoop()
...
def mm_update(mgr, pot_values, button_states):
    ...
    try:
        ...
        # getattr i no mgr.__dict__.get: a CircuitPython, __dict__ COPIA el
        # diccionari de la instància (0,4 ms aquí, 1 ms al teclat) a cada
        # lectura. tests/test_sense_dict_get.py ho vigila.
        status = getattr(mgr, '_status', None)
        if status is None:
            status = {}
            mgr._status = status
        else:
        ...
        # Els testimonis de potes i de BPM per a la Pantalla costaven 1,8 ms a
        # CADA volta (import, supervisor, tres comparacions) quan no hi havia
        # res a dir. Només corren si un pot s'ha mogut des de l'última volta,
        # i el de BPM també una volta de cada vuit (el tempo pot canviar sol).
        _pm = getattr(mgr, '_pots_moguts', None)
        if _pm is None or len(pot_values) < 3:
            _pm = mgr._pots_moguts = [-1, -1, -1]
            potes_moguts = True
        else:
            potes_moguts = (_pm[0] != pot_values[0] or _pm[1] != pot_values[1]
                            or _pm[2] != pot_values[2])
            if potes_moguts:
                ...
                if (raw != efecte_info['last_state'] and
                    ...
                    if raw:
                        ...
                        if held >= mgr.effect_long_press:
                            ...
                            # TAP: capa de comandaments següent (OFF → capa 1 →
                            # … → OFF), de tres en tres de la llista del mode que
                            # sona (o les capes de CC si el mode no en té). Mentre
                            # una capa és activa els potes manen sobre ELLA i el
                            # mode segueix sonant amb els seus potes congelats.
                            lay = _potcfg(mgr)
                            if lay.mode is not mgr.current_mode:
                                lay.set_mode(mgr.current_mode)
        ...
        for efecte_button, efecte_info in mgr.efectes_temporals.items():
            if efecte_info['active'] and efecte_info['tipus'] != 'Harmonia Negativa':
                ...
                if tipus == 'Config Modes':
                    ...
                    try:
                        _lay = _potcfg(mgr)
                        if _lay.mode is not mgr.current_mode:
                            _lay.set_mode(mgr.current_mode)
                        _lay.apply(mgr.effect_manager.midi, pot_values)
                    except Exception:
                        pass
        ...
        if not audio_cfg_active and getattr(mgr, '_audio_cc_cache', None):
        ...
        if not algun_efecte_actiu:
            if mgr.current_mode:
                ...
                if isinstance(button_states, (list, tuple)):
                    filtered = getattr(mgr, '_filtered', None)
