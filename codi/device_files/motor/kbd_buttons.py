...
def build_fn_mappings(kbd):
    ...
    for i in range(min(len(fns), 15)):
        ...
        if f == 'note':
            ...
            # Diverses tecles 'mode' conviuen: cada una engega el seu mode
            # (bank['modes'][i]); un toc a una altra tecla canvia de fons.
...
def process_keyboard_buttons(kbd, button_states):
    ...
    # LIMITAT A 15 aquí dins. Abans qui cridava feia `button_states[:15]`:
    # una llista nova a cada volta del bucle, 177 per segon, i la brossa acaba
    # en recollides de 10-30 ms enmig del que estàs tocant. La llesca no
    # protegia de res que aquest `min` no fes ja.
    n = min(len(button_states), 15)
    # Les llargades, UN cop. Dins dels bucles eren dues crides per iteració —
    # unes trenta per volta, a 177 voltes per segon— i a CircuitPython una
    # crida no és barata: el bucle de sota costava 33 µs per iteració per
    # llegir dos booleans. A més, aquí totes dues eren sempre certes i no
    # protegien de res: `btn_idx` ve de `range(n)` amb `n <= len(button_states)`, i
    # `last_button_states` és `[False] * 15` fixa des del constructor.
    ...
    for btn_idx in range(n):
        ...
        if cur and not prv:
            ...
            if btn_idx == kbd._fn_scale_btn:
                ...
                from motor.accompaniment import handle_button as _accomp_gesture
                held = current_time - getattr(kbd, '_accomp_btn_press_time', current_time)
                _accomp_gesture(kbd, held, current_time)
            elif btn_idx in kbd._fn_fons_btns:
                # Mode de fons sota el teclat (mòdul lazy): tap = engega / capa
                # de comandaments següent / potes al teclat · llarga = atura
                from motor.kbd_fons import handle_button as _fons_gesture
                held = current_time - getattr(kbd, '_fons_btn_press_time', current_time)
                _fons_gesture(kbd, btn_idx, held, getattr(kbd, '_ultims_pots', None))

            elif btn_idx == kbd._fn_looper_btn or btn_idx == kbd._fn_looper_q_btn:
                from motor.kbd_looper import handle_button
                ...
                from motor.kbd_looper import handle_dub_button
    ...
    if kbd.arp_mode_active:
        ...
        note_states = getattr(kbd, '_buf_note_states', None)
        ...
        for slot, btn_idx in enumerate(kbd._note_buttons):
            cur = button_states[btn_idx] if btn_idx < n else False
            prv = lbs[btn_idx] if btn_idx < nl else False
            if cur and not prv:
                ...
                if kbd.loop_state:  # ARMED/RECORDING: capturar l'acord generat
                    from motor.kbd_looper import record_press
                    record_press(kbd, btn_idx, current_time)
            elif not cur and prv:
                if kbd.loop_state:
                    from motor.kbd_looper import record_release
    ...
    # El GATE, per a les dues branques i tant si hi ha tecles premudes com si no.
    #
    # Abans en vivien dues còpies: una al final de la branca del teclat i una
    # altra dins de process_arpeggiator. La de l'arpegiador quedava DARRERE del
    # `return` de «cap tecla premuda», o sigui que deixar anar les tecles amb
    # l'arpegiador actiu congelava el gate. Si el cicle havia quedat a la fase
    # baixa, el CC11 es quedava a zero -Expression zero, canal MUT- i no
    # tornava fins que tornaves a prémer una tecla.
    #
    # El pitjor no era el silenci sinó que res no ho deia: les notes seguien
    # sortint i la Pantalla les mostrava. Es veia tocar i no se sentia.
    #
    # El gate no és cosa de l'arpegiador ni del teclat: és un modulador de la
    # sortida i ha de córrer sempre que el mode corre.
    if kbd.gate_enabled:
