...
class MIDI:
    ...
    def send(self, msg: MIDIMessage, channel: Optional[int] = None) -> None:
        ...
        """
        # PATCH TECLA: si el missatge JA porta canal (msg.channel is not None)
        # es respecta — l'upstream el sobreescrivia sempre amb out_channel, cosa
        # que trencava els enviaments multicanal (MixRunner, acompanyament 8-11,
        # modes multicanal) que el simulador sí que respecta. El canal explícit
        # de send(msg, channel) continua manant sobre tot.
        if isinstance(msg, MIDIMessage):
            if channel is not None:
                msg.channel = channel
            elif msg.channel is None:
                msg.channel = self.out_channel
            ...
            for each_msg in msg:
                ...
                elif each_msg.channel is None:
                    each_msg.channel = self.out_channel
                data.extend(
