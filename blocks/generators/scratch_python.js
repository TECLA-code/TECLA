...
Blockly.Python.forBlock['scratch_when_flag_clicked'] = function(block) {
  // Aquest bloc marca l'inici del programa
  return '# Programa iniciat (bandera verda)\n';
};

Blockly.Python.forBlock['scratch_when_key_pressed'] = function(block) {
  const key = block.getFieldValue('KEY');
  Blockly.Python.definitions_['import_usb_hid'] = 'try:\n    import usb_hid\n    from adafruit_hid.keyboard import Keyboard\n    kbd = Keyboard(usb_hid.devices)\nexcept:\n    kbd = None';
  return `# Event: Quan es premi tecla ${key}\n`;
};

Blockly.Python.forBlock['scratch_broadcast'] = function(block) {
...
Blockly.Python.forBlock['scratch_when_receive'] = function(block) {
  const message = block.getFieldValue('MESSAGE');
  return `# Event: Quan rebi "${message}"\nif check_broadcast("${message}"):\n    clear_broadcast("${message}")\n`;
};

Blockly.Python.forBlock['scratch_when_button_pressed'] = function(block) {
...
Blockly.Python.forBlock['scratch_say'] = function(block) {
...
Blockly.Python.forBlock['scratch_think'] = function(block) {
  const message = Blockly.Python.valueToCode(block, 'MESSAGE', Blockly.Python.ORDER_NONE) || '""';
  const seconds = Blockly.Python.valueToCode(block, 'SECONDS', Blockly.Python.ORDER_NONE) || '2';
  Blockly.Python.definitions_['import_time'] = 'import time';
  return `print("(Pensant: " + str(${message}) + ")")\ntime.sleep(${seconds})\n`;
};

Blockly.Python.forBlock['scratch_show'] = function(block) {
  return '# Activar sortida/display\n';
};

Blockly.Python.forBlock['scratch_hide'] = function(block) {
  return '# Desactivar sortida/display\n';
};

Blockly.Python.forBlock['scratch_change_size'] = function(block) {
  const change = Blockly.Python.valueToCode(block, 'CHANGE', Blockly.Python.ORDER_NONE) || '10';
  return `# Canviar mida per ${change}\n`;
};

Blockly.Python.forBlock['scratch_set_size'] = function(block) {
...
Blockly.Python.forBlock['scratch_play_sound'] = function(block) {
...
Blockly.Python.forBlock['scratch_play_sound_until_done'] = function(block) {
  const sound = block.getFieldValue('SOUND');
...
Blockly.Python.forBlock['scratch_stop_all_sounds'] = function(block) {
  return '# Aturar tots els sons\nfor i in range(128):\n    midi.send(NoteOff(i, 0))\n';
};

Blockly.Python.forBlock['scratch_change_volume'] = function(block) {
  const change = Blockly.Python.valueToCode(block, 'CHANGE', Blockly.Python.ORDER_NONE) || '10';
  Blockly.Python.definitions_['volume_var'] = 'global_volume = 100';
  return `global_volume = max(0, min(100, global_volume + ${change}))\n`;
};

Blockly.Python.forBlock['scratch_set_volume'] = function(block) {
...
Blockly.Python.forBlock['scratch_touching'] = function(block) {
...
Blockly.Python.forBlock['scratch_key_pressed'] = function(block) {
  const key = block.getFieldValue('KEY');
  return [`False  # Tecla ${key} (requereix connexió PC)`, Blockly.Python.ORDER_ATOMIC];
};

Blockly.Python.forBlock['scratch_mouse_x'] = function(block) {
  return ['0  # Mouse X (requereix connexió PC)', Blockly.Python.ORDER_ATOMIC];
};

Blockly.Python.forBlock['scratch_mouse_y'] = function(block) {
  return ['0  # Mouse Y (requereix connexió PC)', Blockly.Python.ORDER_ATOMIC];
};

Blockly.Python.forBlock['scratch_loudness'] = function(block) {
...
Blockly.Python.forBlock['scratch_timer'] = function(block) {
  Blockly.Python.definitions_['import_time'] = 'import time';
...
Blockly.Python.forBlock['scratch_reset_timer'] = function(block) {
  Blockly.Python.definitions_['import_time'] = 'import time';
  return 'start_time = time.monotonic()\n';
};

Blockly.Python.forBlock['scratch_current_time'] = function(block) {
...
Blockly.Python.forBlock['scratch_days_since_2000'] = function(block) {
  Blockly.Python.definitions_['import_time'] = 'import time';
...
Blockly.Python.forBlock['scratch_join'] = function(block) {
...
Blockly.Python.forBlock['scratch_letter_of'] = function(block) {
...
Blockly.Python.forBlock['scratch_length_of'] = function(block) {
  const string = Blockly.Python.valueToCode(block, 'STRING', Blockly.Python.ORDER_MEMBER) || '""';
  return [`len(str(${string}))`, Blockly.Python.ORDER_FUNCTION_CALL];
};

Blockly.Python.forBlock['scratch_contains'] = function(block) {
...
Blockly.Python.forBlock['scratch_mod'] = function(block) {
...
Blockly.Python.forBlock['scratch_round'] = function(block) {
...
Blockly.Python.forBlock['scratch_mathop'] = function(block) {
...
Blockly.Python.forBlock['scratch_custom_block'] = function(block) {
  const name = block.getFieldValue('NAME');
  return `${name}()\n`;
};

Blockly.Python.forBlock['scratch_define_custom'] = function(block) {
  const name = block.getFieldValue('NAME');
...
Blockly.Python.forBlock['scratch_change_var'] = function(block) {
  const _vid = block.getFieldValue('VAR');
  const variable = _vid ? Blockly.Python.nameDB_.getName(_vid, Blockly.Names.NameType.VARIABLE) : 'variable';
  const value = Blockly.Python.valueToCode(block, 'VALUE', Blockly.Python.ORDER_NONE) || '1';
  return `${variable} = ${variable} + ${value}\n`;
};

Blockly.Python.forBlock['scratch_show_variable'] = function(block) {
  const _vid = block.getFieldValue('VAR');
  const variable = _vid ? Blockly.Python.nameDB_.getName(_vid, Blockly.Names.NameType.VARIABLE) : 'variable';
  return `print(f"${variable} = {${variable}}")\n`;
};

Blockly.Python.forBlock['scratch_hide_variable'] = function(block) {
  return '# Amagar variable del display\n';
};

// ==================== CONTROL ====================

Blockly.Python.forBlock['scratch_stop'] = function(block) {
...
Blockly.Python.forBlock['scratch_when_start_as_clone'] = function(block) {
  return '# Event: Quan comenci com a clon\n';
};

Blockly.Python.forBlock['scratch_create_clone'] = function(block) {
...
Blockly.Python.forBlock['scratch_delete_clone'] = function(block) {
