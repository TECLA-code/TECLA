...
Feedback Visual:
- LED pisca durant comprovació
- LED encès = Mode Blocks actiu
- LED apagat = Mode TECLA normal
"""
import board
import digitalio
import time
...
def select_mode():
    ...
    for btn_num, button_io in buttons_io.items():
        if check_long_press(button_io):
            ...
            print(f"   Carregant {config['file']}...")
            
            try:
                # Importar el mòdul corresponent
                module_name = config['file'].replace('.py', '')
                __import__(module_name)
                return
            except ImportError as e:
                print(f"\n⚠️  ERROR: {config['file']} no trobat")
                print(f"   Crea aquest projecte amb TECLA Blocks primer")
                print(f"   Detalls: {e}\n")
                blink_error()
                return
